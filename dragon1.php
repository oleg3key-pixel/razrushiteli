...
header('location:dragon1.php');exit;}
...
$ten = mysql_query('SELECT * FROM `dragon_hero_1` WHERE `users` = "'.$user['id'].'"');      
...
if($aid['hp']>0){   if(!$ten) {
  ...
  mysql_query('INSERT INTO `dragon_hero_1` (users,hp,strong,def,name) VALUES ("'.$user['id'].'","'.$user['hp'].'","'.$user['str'].'","'.$user['def'].'","'.$name.'")');
...
$nagrada1 =  mt_rand(1,1);
$nagrada2 =  mt_rand(100,100);
$nagrada3 =  mt_rand(100,100);
...
if($aid['status']==0){ header('location:dragon1.php');exit;}


if($aid['ten']<=0){ header('location:dragon1.php');exit;}
if($ten['vit']<=0){ header('location:dragon1.php');exit;}
...
mysql_query('UPDATE `dragon_hero_1` SET `koef` = `koef`+ '.$udar.' WHERE `users` = '.$user[id].'');    
...
$ot = ceil($ot);
if($ot>=$ten['vit']){ mysql_query('UPDATE `aid` SET `ten` =`ten`-1 WHERE `id`= 1'); }


mysql_query('UPDATE `dragon_hero_1` SET `vit` = `vit`- '.$ot.' WHERE `users` = '.$user[id].'');    
...
$_SESSION['log']='<font color=green>Вы Ударили Зеленого дракона на '.$udar.'</font>'; 
header('location:dragon1.php');exit;
}
...
$prot= mysql_fetch_assoc(mysql_query("SELECT * FROM `dragon_hero_1` WHERE `hp`>0 ORDER BY RAND() LIMIT 1"));
...
mysql_query('UPDATE `dragon_hero_1` SET `koef` = `koef`+ '.$plus.' WHERE `users` = '.$user[id].'');    

$_SESSION['log']='<font color=green>Вы убили '.$prot['name'].'</font>';}

else{   
mysql_query('UPDATE `dragon_hero_1` SET `koef` = `koef`+ '.$udar.' WHERE `users` = '.$user[id].'');    
mysql_query('UPDATE `dragon_hero_1` SET `vit` = `vit`- '.$udar.' WHERE `users` = '.$prot[users].'');    
...
$geroi = mysql_fetch_assoc(mysql_query("SELECT * FROM `dragon_hero_1` ORDER BY `koef` DESC LIMIT 1")); 
...
mysql_query("truncate table `dragon_hero_1` ");
...
<b> <font color=orange>На зелёного дракона напало: <?=$aid['ten']?> героев</font></b>
     ...
     <div class="wr3">
      <div class="wr4">
       <div class="wr5">
        ...
        <img src="/images/drago/1.png" class=icon width=38>

             </div>
            </div> 
           </div> 
           <div class="ml68 mt10 mb10 mr10 sh"> 
            <span class="lwhite tdn">Зелёный дракоша</span> 
            <div class="small mb2"> 
...
?><a class=mbtn mb2 href='dragon1.php?bat' class='btn'><span class='end'><span class='label'>Атаковать</a></span></span><?
...
?><div class="bdr bg_blue mb2"><div class="wr1"><div class="wr2"><div class="wr3"><div class="wr4"><div class="wr5"><div class="wr6"><div class="wr7"><div class="wr8">
	<div class="ml10 mt5 mr10 mb5 cntr">
	    <div class='content'><font color=orange> Зелёного дракона добил: </font> <a href='/user/<?=$hr['id']?>/'><?=$hr['login']?></a></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>		<div class="clb"></div>
	</div></div></div></div></div></div></div></div>
