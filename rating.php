...
$q = mysql_query('SELECT * FROM `users` ORDER BY `str`+`vit`+`def` DESC LIMIT '.$start.', '.$max.'');
...
echo ' <div class="hr_arr mlr10 mb-3">
            ...
            <div class="pt3 mlr10 cntr"> 
             <div class="fl w25 inbl spr2bg r"> 
              <span class="slct"><span class="send"><span class="sttl"> Игроки </span></span></span> 
             </div> 
             <div class="fl w25 inbl spr2bg r"> 
            ...
            <table class="cntr wa mlra mb10"> 
             <tbody>
              <tr> 
               ...
               <td class="p5">Имя</td> 
               <td class="p5">Параметры</td> 
              </tr>';

           while($row = mysql_fetch_array($q)) {//Вывод игроков
            $a=array($row['vit'],$row['str'],$row['def']);//Додаем параметры
$i++;
