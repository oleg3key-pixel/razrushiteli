...
$aid = mysql_query('SELECT * FROM `aid` WHERE `id`=5');       
   ...
   $aid = mysql_fetch_array($aid);
        ...
        $hp_aid = round(100/(1500000/$aid['hp']));
...
header('location:dragon5.php');exit;}
...
$ten = mysql_query('SELECT * FROM `dragon_hero_5` WHERE `users` = "'.$user['id'].'"');      
...
if($aid['hp']>0){   if(!$ten) {
  ...
  mysql_query('INSERT INTO `dragon_hero_5` (users,hp,strong,def,name) VALUES ("'.$user['id'].'","'.$user['hp'].'","'.$user['str'].'","'.$user['def'].'","'.$name.'")');


mysql_query('UPDATE `aid` SET `ten` =`ten`+1 WHERE `id`= 5');
...
$nagrada1 =  mt_rand(5,5);
$nagrada2 =  mt_rand(500,500);
$nagrada3 =  mt_rand(500,500);
...
if($aid['status']==0){ header('location:dragon5.php');exit;}


if($aid['ten']<=0){ header('location:dragon5.php');exit;}
if($ten['vit']<=0){ header('location:dragon5.php');exit;}
...
mysql_query('UPDATE `aid` SET `hp` = `hp`-'.$udar.' WHERE `id`=5');   
...
mysql_query('UPDATE `dragon_hero_5` SET `koef` = `koef`+ '.$udar.' WHERE `users` = '.$user[id].'');    


if($udar>=$aid['hp']){   


mysql_query('UPDATE `aid` SET `voln` =1,`hp`=0 WHERE `id`= 5');
...
$ot = ceil($ot);
if($ot>=$ten['vit']){ mysql_query('UPDATE `aid` SET `ten` =`ten`-1 WHERE `id`= 5'); }


mysql_query('UPDATE `dragon_hero_5` SET `vit` = `vit`- '.$ot.' WHERE `users` = '.$user[id].'');    

mysql_query('UPDATE `aid` SET `hp` = `hp`+ '.$ot.' WHERE `id` = 5');    
...
$_SESSION['log']='<font color=green>Вы Ударили золотого дракона на '.$udar.'</font>'; 
header('location:dragon5.php');exit;
}
...
$prot= mysql_fetch_assoc(mysql_query("SELECT * FROM `dragon_hero_5` WHERE `hp`>0 ORDER BY RAND() LIMIT 1"));
...
mysql_query('UPDATE `dragon_hero_5` SET `koef` = `koef`+ '.$plus.' WHERE `users` = '.$user[id].'');    

$_SESSION['log']='<font color=green>Вы убили '.$prot['name'].'</font>';}

else{   
mysql_query('UPDATE `dragon_hero_5` SET `koef` = `koef`+ '.$udar.' WHERE `users` = '.$user[id].'');    
mysql_query('UPDATE `dragon_hero_5` SET `vit` = `vit`- '.$udar.' WHERE `users` = '.$prot[users].'');    

$_SESSION['log']='<font color=green>Вы Ударили '.$prot['name'].' на '.$udar.'</font>';} 

$new = mysql_fetch_assoc(mysql_query("SELECT * FROM `aid` WHERE `id` = 5")); 
if($new['ten']<=0){




$geroi = mysql_fetch_assoc(mysql_query("SELECT * FROM `dragon_hero_5` ORDER BY `koef` DESC LIMIT 1")); 
...
`time` = "'.(time() + 21600).'" , `gr`="'.$geroi['users'].'" WHERE `id` = 5');
...
mysql_query("truncate table `dragon_hero_5` ");
...
<b> <font color=orange>На золотого дракона напало: <?=$aid['ten']?> героев</font></b>
     ...
     <div class="wr3">
      <div class="wr4">
       <div class="wr5">
        ...
        <img src="/images/drago/5.png" class=icon width=38>

             </div>
            </div> 
           </div> 
           <div class="ml68 mt10 mb10 mr10 sh"> 
            <span class="lwhite tdn">Золотой дракоша</span> 
            <div class="small mb2"> 
...
?><a class=mbtn mb2 href='dragon5.php?bat' class='btn'><span class='end'><span class='label'>Атаковать</a></span></span><?
...
?><div class="bdr bg_blue mb2"><div class="wr1"><div class="wr2"><div class="wr3"><div class="wr4"><div class="wr5"><div class="wr6"><div class="wr7"><div class="wr8">
	<div class="ml10 mt5 mr10 mb5 cntr">
	    <div class='content'><font color=orange> Золотого дракона добил: </font> <a href='/user/<?=$hr['id']?>/'><?=$hr['login']?></a></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div></div>		<div class="clb"></div>
	</div></div></div></div></div></div></div></div>
